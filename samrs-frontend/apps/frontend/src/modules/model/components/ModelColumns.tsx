...
export const createModelColumns = (opts: { onEdit: (model: Model) => void; onDelete: (id: number) => void }): ColumnDef<Model>[] => [
  { accessorKey: "name", header: "Name", cell: ({ row }) => <div className="font-medium">{row.original.name}</div> },
  { accessorKey: "brand", header: "Brand", cell: ({ row }) => row.original.brand?.name || "-" },
